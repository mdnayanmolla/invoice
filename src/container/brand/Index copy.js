...
function ViewPage() {
  ...
  const { crud, isLoading } = useSelector((state) => {
    return {
      crud: state.BrandCrud.data?.data ?? [],
      isLoading: state.BrandCrud.loading,
    };
  });
  ...
  const onHandleSearch = (e) => {
    dispatch(brandDataSearch(e.target.value, crud));
  };
          ...
          <div className="min-w-[150px] text-end -m-2">
            ...
            <Link className="inline-block m-2" to="#">
              <Popconfirm
                ...
                cancelText="No"
              >
                <Link className="inline-flex items-center text-light dark:text-white60 hover:text-danger" to="#">
                  <UilTrash className=" w-[16px]" />
                </Link>
              </Popconfirm>
            </Link>
          </div>
